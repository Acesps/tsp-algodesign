...
def tryit(G,n,nodes):
	...
	for i in range(4,len(nodes)+2):
		...
		#print list_ans
		output += "adding node" + str(i) + "\n" + str(list_ans) + "\n" + "distance :"+str(distance) + "\n"
		# print " g is "
		# print g
		# print "path is "
		# print list_ans[0]
		for path in list_ans:
			divided_distance = ""
			d = 0
			for i in range(0,len(path)-1):
				divided_distance+=str(g[path[i]-1][path[i+1]-1]) + " + "
				d += g[path[i]-1][path[i+1]-1]
			output += "calculation: " + divided_distance + "\n" + str(d) + "\n"		
	return distance, list_ans, output
...
while z < 1000:
	...
	n = randint(6,10) #int(raw_input("enter the number of nodes")); #
	print str(z) +" " +  str(n) 
	...
	for i in range(1,n+1):
	    ...
	    for j in range(i+1,n+1):
	        weight =randint(251,500) # g[i-1][j-1] #
	...
	#storing the start time 
	start_time = time.time()
	distance2,path2,output = tryit(G,n,nodes)
	# Gnew=nx.Graph()
	# for i in range(0,len(path)-1):
	...
	if(path1 != path2):
		...
		for x in range(4,n):
			nodes=[int(a) for a in range(2,x+1)]
			distance3,path3 = travllingSalesmanProblem(G,n,nodes)
			print "number of nodes: " + str(x) + " sequence: "
			print path3
			print "distance: " + str(distance3)
			for path in path3:
				divided_distance=""
				d=0
				for i in range(0,len(path)-1):
					divided_distance+=str(g[path[i]-1][path[i+1]-1]) + " + "
					d += g[path[i]-1][path[i+1]-1]
				print "calculation: " + divided_distance
				print d 
		print "number of nodes: " + str(n) + " sequence: "
		print path1;
